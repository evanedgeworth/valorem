          ...
          <div className="flex flex-1 justify-between items-center" key={noti.id}>
            <div className="flex items-center">
              ...
              <div className="ms-3 text-sm font-normal">
                <div className="text-sm font-semibold text-gray-900 dark:text-white">{noti.notifier.first_name + " " + noti.notifier.last_name}</div>
                <div className="text-sm font-normal">{noti.message}</div>
                <span className="text-xs font-medium text-blue-600 dark:text-blue-500">{moment(noti.created_at).fromNow()}</span>
              </div>
            </div>
            <IoMdClose size={25} className=" text-gray-500 cursor-pointer" />
          </div>
        ))}
      </section>
    </>
