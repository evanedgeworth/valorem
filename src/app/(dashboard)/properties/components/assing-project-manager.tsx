...
function ListUser({ property, close }: { property: Property, close: () => void }) {
  ...
  const items: UserOrganization[] = useMemo(() => {
    ...
    return result.filter(item => ["JUNIOR PROJECT MANAGER", "PROJECT MANAGER"].includes(item.role?.roleName || '') && (searchInput ? item.name.toLowerCase().includes(searchInput.toLowerCase()) : true));
