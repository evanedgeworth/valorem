...
export default function NewPropertyModal({ showModal, setShowModal }: { showModal: boolean; setShowModal: (value: boolean) => void }) {
  ...
  return (
    <div ref={rootRef}>
      ...
      <Modal show={showModal} size="md" dismissible popup onClose={() => setShowModal(false)} root={rootRef.current ?? undefined}>
        <Modal.Header className="items-center px-6 pt-4">
          <h3 className="text-xl font-medium">New Property</h3>
        </Modal.Header>
        <Modal.Body>
          <PropertyForm onSubmit={handleCreateProperty} isLoading={isLoading} onClose={() => setShowModal(false)} />
        </Modal.Body>
      </Modal>
    </div>
  );
